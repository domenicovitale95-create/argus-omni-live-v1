...
export default async function handler(req,res){
  ...
  if(!storageReady())return res.status(503).json({version:'FAST-CYCLE-HEALTH-2',status:'CRITICAL',backupRequired:true,error:'Storage unavailable'});
  const state=await readJsonFresh(STATE,null);
  if(!state)return res.status(200).json({version:'FAST-CYCLE-HEALTH-2',generatedAt:new Date().toISOString(),status:'WAITING',backupRequired:true,ageMinutes:null,lastCycle:null,policy:{lateAfterMinutes:LATE_AFTER_MINUTES,staleAfterMinutes:STALE_AFTER_MINUTES,readOnly:true,providerCalls:false,persistentWrites:false}});
  ...
  return res.status(200).json({
    version:'FAST-CYCLE-HEALTH-2',generatedAt:new Date().toISOString(),status,backupRequired,ageMinutes:age,
