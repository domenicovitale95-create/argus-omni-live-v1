...
const PATHS={
  deployment:'argus/health/deployment-verification.json',
...
function errorDiagnostics(errors){
  ...
  for(const row of rows){
    const key=(row.split(':')[0]||'UNKNOWN').trim().toUpperCase().replace(/[^A-Z0-9]+/g,'_')||'UNKNOWN';
...
export default async function handler(req,res){
  ...
  const [deployment,selfImprovement,governance,scheduler,autopilot,ledgerCron]=await Promise.all(Object.values(PATHS).map(p=>readJson(p,null).catch(()=>null)));
  ...
  const components={
    deployment:{status:deployment?.status||'UNKNOWN',ageMinutes:depAge,freshness:freshness(depAge,20),commit:deployment?.vercel?.gitCommitSha||null,failures:deployment?.critical?.failed??null},
  ...
  const vals=Object.values(components),criticalBad=[components.deployment,components.autopilot,components.scheduler,components.ledger].some(x=>['DEGRADED','BLOCKED'].includes(x.status)||x.freshness==='STALE'),unknown=vals.filter(x=>x.status==='UNKNOWN').length,stale=vals.filter(x=>x.freshness==='STALE').length;
  const status=criticalBad?'ACTION_REQUIRED':unknown>=3?'INCOMPLETE':stale?'ATTENTION':'HEALTHY';
  const priorities=[];
  if(components.deployment.status!=='READY')priorities.push('VERIFY_DEPLOYMENT');
  ...
  return res.status(200).json({
    ...
    policy:{readOnly:true,noProviderQuotaSpend:true,snapshotsOnly:true,designedForFastDeveloperTriage:true,errorSamplesAreStoredSnapshotData:true,protectedPreview401sDoNotImplyModelDegradation:true}
  });
}
