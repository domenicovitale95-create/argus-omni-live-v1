...
export default async function handler(req,res){
  ...
  if(!storageReady())return res.status(503).json({version:'DECISION-INTEGRITY-AUDIT-3',status:'DEGRADED',error:'Storage unavailable',violations:[]});
  ...
  for(const r of rows){
    ...
    if(isLive&&eligible){if(c.modelVersion!==LIVE_MODEL)push('LIVE_ELIGIBLE_WITHOUT_CANONICAL_MODEL','CRITICAL',{modelVersion:c.modelVersion||null});if(c.validationStatus!=='VALIDATED')push('LIVE_ELIGIBLE_WITHOUT_EMPIRICAL_VALIDATION','CRITICAL',{validationStatus:c.validationStatus||null});if(n(c.observedAgeSeconds,null)!=null){const maxAge=n(r.minute,0)>=80?45:75;if(n(c.observedAgeSeconds)>maxAge)push('LIVE_ELIGIBLE_WITH_STALE_SNAPSHOT','CRITICAL',{observedAgeSeconds:n(c.observedAgeSeconds),maxAgeSeconds:maxAge})}}
