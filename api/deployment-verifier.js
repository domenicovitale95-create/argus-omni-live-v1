...
async function latestSnapshot(){const scoped=await readJson(OUT,null);if(scoped)return scoped;if(ENV==='production')return readJson(LEGACY_OUT,{version:'DEPLOYMENT-VERIFIER-4',status:'UNKNOWN',generatedAt:null,snapshotScope:ENV});return{version:'DEPLOYMENT-VERIFIER-4',status:'UNKNOWN',generatedAt:null,snapshotScope:ENV}}
export default async function handler(req,res){
  ...
  const checks=await Promise.all(CRITICAL.map(p=>get(base,p,auth))),failed=checks.filter(x=>!x.ok),semanticFails=checks.filter(x=>SEMANTIC_BLOCK.has(String(x.semanticStatus||'').toUpperCase())),selfFail=!selfTest||!['HEALTHY','SLOW'].includes(String(selfTest.status||''));
  let status='READY';
  ...
  const state={version:'DEPLOYMENT-VERIFIER-4',generatedAt:new Date().toISOString(),status,snapshotScope:ENV,checkedBase:new URL(base).host,vercel:{environment:process.env.VERCEL_ENV||null,gitCommitSha:process.env.VERCEL_GIT_COMMIT_SHA||null,gitBranch:process.env.VERCEL_GIT_COMMIT_REF||null,deploymentId:process.env.VERCEL_DEPLOYMENT_ID||null},selfTest:{status:selfTest?.status||'UNAVAILABLE',checked:selfTest?.checked??null,failures:selfTest?.failures?.length??null,waiting:selfTest?.waiting?.length??null,slow:selfTest?.slow?.length??null},critical:{checked:checks.length,failed:failed.length,semanticFails:semanticFails.length,checks},policy:{readOnlyChecks:true,persistentSnapshot:true,environmentScopedSnapshot:true,previewCannotOverwriteProductionSnapshot:true,providerQuotaSpendAllowed:false,mutatingEndpointsExcluded:true,automaticRollback:false,automaticPromotion:false,autonomyIsCritical:true,readyRequiresCriticalHealth:true}};
