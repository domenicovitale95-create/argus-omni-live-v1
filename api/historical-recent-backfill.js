...
const NEVER_STARTED=new Set(['TBD','NS']);
const NEVER_STARTED_GRACE_MS=3*60*60*1000;
...
function staleNeverStarted(f,date,now=Date.now()){
  const status=String(f?.fixture?.status?.short||'');
  if(!NEVER_STARTED.has(status))return false;
  const ts=Number(f?.fixture?.timestamp),parsed=Date.parse(f?.fixture?.date||'');
  const scheduledMs=Number.isFinite(ts)&&ts>0?ts*1000:Number.isFinite(parsed)?parsed:null;
  if(scheduledMs!=null)return now-scheduledMs>=NEVER_STARTED_GRACE_MS;
  return date<addDays(brusselsDate(),-1);
}
async function fetchDate(date){
  const key=process.env.API_FOOTBALL_KEY;if(!key)throw new Error('API_FOOTBALL_KEY is not configured');
  const r=await fetch(`${API_BASE}/fixtures?date=${date}&timezone=${encodeURIComponent(TZ)}`,{headers:{'x-apisports-key':key,Accept:'application/json'}});const q=quota(r.headers);
  if(!r.ok)throw new Error(`API-Football HTTP ${r.status}`);const j=await r.json();if(j?.errors&&Object.keys(j.errors).length)throw new Error(`API-Football: ${JSON.stringify(j.errors)}`);
  const all=j.response||[],unresolved=all.filter(f=>ACTIVE_OR_SCHEDULED.has(String(f?.fixture?.status?.short||''))),staleExcluded=unresolved.filter(f=>staleNeverStarted(f,date)),blocking=unresolved.filter(f=>!staleNeverStarted(f,date));
  return{rows:all.map(normalize).filter(Boolean),quota:q,settlementReady:blocking.length===0,activeUnsettled:blocking.length,activeStatuses:[...new Set(blocking.map(f=>String(f?.fixture?.status?.short||'UNKNOWN')))],staleNeverStartedExcluded:staleExcluded.map(f=>({fixtureId:Number(f?.fixture?.id)||null,status:String(f?.fixture?.status?.short||'UNKNOWN'),kickoff:f?.fixture?.date||null})).slice(0,100)};
}
...
export default async function handler(req,res){
  ...
  let attempted=0,processed=0,newFixtures=0,providerCalls=0,lastQuota=null,stopReason=null;const errors=[],deferred=[],staleNeverStartedExcluded=[];
  for(const date of pending){
    ...
    try{
      ...
      if(out.staleNeverStartedExcluded?.length)staleNeverStartedExcluded.push({date,fixtures:out.staleNeverStartedExcluded});
      ...
      const excludedCount=out.staleNeverStartedExcluded?.length||0;
      shard.dates[date]={complete:true,fixtures:out.rows.length,savedAt:new Date().toISOString(),settlementReady:true,staleNeverStartedExcluded:excludedCount};shard.updatedAt=new Date().toISOString();index.dates[date]={complete:true,fixtures:out.rows.length,month:date.slice(0,7),savedAt:shard.updatedAt,settlementReady:true,staleNeverStartedExcluded:excludedCount};index.months[date.slice(0,7)]={path,updatedAt:shard.updatedAt};dirty.add(path);processed++;
  ...
  const coverageDates=datesToYesterday(),recentCompleteDays=(()=>{let c=0;for(const d of coverageDates){if(index.dates[d]?.complete)c++;else break}return c})();index.version='HISTORICAL-RECENT-INDEX-2';index.migrationFloor=MIGRATION_FLOOR;index.updatedAt=new Date().toISOString();index.requiredThrough=coverageDates[0]||null;index.recentCompleteDays=recentCompleteDays;index.lastBudget={observedAt:index.updatedAt,before:beforeBudget,after:lastQuota?budget(lastQuota):beforeBudget,providerCalls,stopReason};index.lastDeferred=deferred;index.lastStaleNeverStartedExcluded=staleNeverStartedExcluded;await writeJson(INDEX,index);
  const status=errors.length?'PARTIAL':stopReason?'PAUSED_AFTER_PROGRESS':deferred.length?'DEFERRED_UNSETTLED':'OK';
  return res.status(errors.length?207:200).json({ok:errors.length===0,version:'HISTORICAL-RECENT-BACKFILL-4',status,migrationFloor:MIGRATION_FLOOR,requiredThrough:index.requiredThrough,attemptedDates:attempted,processedDates:processed,providerCalls,newFixtures,pendingBeforeRun:pending.length,recentCompleteDays,deferred,staleNeverStartedExcluded,quota:lastQuota,budgetBefore:beforeBudget,budgetAfter:lastQuota?budget(lastQuota):beforeBudget,maxCallsByBudget,stopReason,errors,policy:{monthlyShards:true,noLegacyRewrite:true,recentFirst:true,noGapFilling:true,quotaAware:true,persistentIndex:true,completedDatesNeverRefetched:true,activeStartedFixturesAlwaysBlock:true,staleNeverStartedFixturesMayBeExcluded:true,staleNeverStartedGraceHours:NEVER_STARTED_GRACE_MS/3600000,maxBatch:MAX_BATCH,learningReserveRatio:LEARNING_RESERVE_RATIO,operationalReserveRatio:OPERATIONAL_RESERVE_RATIO,minuteReserve:MINUTE_RESERVE,operationalTrafficPriority:true,providerGuardRespected:true,automaticResume:true,noSyntheticResults:true}})
}
