...
function shouldStop(q){const r=reserve(q);return(q?.dailyRemaining!=null&&q.dailyRemaining<=r)||(q?.minuteRemaining!=null&&q.minuteRemaining<=3)}
...
export default async function handler(req,res){
  ...
  if(String(req.query?.dryRun||'')==='1')return res.status(200).json({ok:true,version:'HISTORICAL-RECENT-BACKFILL-2',status:'DRY_RUN',today,migrationFloor:MIGRATION_FLOOR,requiredThrough:allDates[0]||null,pendingDates:pending.slice(0,requested),pendingCount:pending.length,providerCalls:0,writes:0,policy:{dryRun:true,providerQuotaSpend:false,persistentWrites:false,monthlyShards:true,noLegacyRewrite:true}});
  if(guard?.date===today&&guard?.exhausted)return res.status(200).json({ok:true,version:'HISTORICAL-RECENT-BACKFILL-2',status:'PAUSED_QUOTA_GUARD',today,processedDates:0,providerCalls:0,guardDate:guard.date,policy:{failClosed:true,noProviderQuotaSpend:true,automaticResumeOnNewBrusselsDate:true}});
  ...
  let attempted=0,processed=0,newFixtures=0,providerCalls=0,lastQuota=null;const errors=[],deferred=[];
  for(const date of pending){if(attempted>=requested)break;try{attempted++;const out=await fetchDate(date);providerCalls++;lastQuota=out.quota;if(!out.settlementReady){deferred.push({date,activeUnsettled:out.activeUnsettled,statuses:out.activeStatuses});if(shouldStop(lastQuota))break;continue}const shard=await month(date),path=monthPath(date);for(const row of out.rows){const k=String(row.fixtureId);if(!shard.fixtures[k])newFixtures++;shard.fixtures[k]=row}shard.dates[date]={complete:true,fixtures:out.rows.length,savedAt:new Date().toISOString(),settlementReady:true};shard.updatedAt=new Date().toISOString();index.dates[date]={complete:true,fixtures:out.rows.length,month:date.slice(0,7),savedAt:shard.updatedAt,settlementReady:true};index.months[date.slice(0,7)]={path,updatedAt:shard.updatedAt};dirty.add(path);processed++;if(shouldStop(lastQuota))break}catch(e){errors.push({date,error:e.message});break}}
  ...
  const status=errors.length?'PARTIAL':deferred.length?'DEFERRED_UNSETTLED':'OK';
  return res.status(errors.length?207:200).json({ok:errors.length===0,version:'HISTORICAL-RECENT-BACKFILL-2',status,migrationFloor:MIGRATION_FLOOR,requiredThrough:index.requiredThrough,attemptedDates:attempted,processedDates:processed,providerCalls,newFixtures,pendingBeforeRun:pending.length,recentCompleteDays,deferred,quota:lastQuota,reserve:reserve(lastQuota),errors,policy:{monthlyShards:true,noLegacyRewrite:true,recentFirst:true,noGapFilling:true,quotaAware:true,persistentIndex:true,completedDatesNeverRefetched:true,activeOrScheduledDatesDeferred:true,maxBatch:MAX_BATCH}})
}
