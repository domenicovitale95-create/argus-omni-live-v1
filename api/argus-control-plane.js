...
function deriveNextAction({developer,readiness,tracking,shards}){
...
export default async function handler(req,res){
  ...
  const [site,developer,readiness,tracking,index]=await Promise.all([
  ...
  const nextAction=deriveNextAction({developer,readiness,tracking,shards});
  return res.status(200).json({
    ok:status==='HEALTHY',
    version:'ARGUS-CONTROL-PLANE-1',
    ...
    systems,
    nextAction,
    evidence:{siteHealth:site.body,developerHealth:developer.body,autopilotReadiness:readiness.body,trackingHealth:tracking.body},
    policy:{readOnly:true,noProviderQuotaSpend:true,failClosed:true,legacyProgressPreserved:true,productionAndMaturitySeparated:true,internalHealthyLabelsRequireCrossChecks:true,designedForAutonomousTriage:true}
  });
}
