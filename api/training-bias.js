...
function auditComplete(b){const source=Boolean(b?.modelSource||b?.source||b?.marketFusion?.mode);return has(b?.modelProbability)&&has(b?.marketProbability)&&has(b?.rawEdgePct)&&source&&Boolean(b?.marketFusion)}
...
function diagnostics(rows,{cohort='ALL'}={}){
...
export default async function handler(req,res){
  ...
  const state=await readJsonFresh(STATE,null),rows=Object.values(state?.bets||{}),audited=rows.filter(auditComplete),legacy=rows.filter(b=>!auditComplete(b));
  const all=diagnostics(rows,{cohort:'ALL_TRAINING'}),prospective=diagnostics(audited,{cohort:'PROSPECTIVE_AUDITED'}),legacyDiag=diagnostics(legacy,{cohort:'LEGACY_INCOMPLETE'});
  return res.status(200).json({ok:true,version:'TRAINING-BIAS-DIAGNOSTIC-2-COHORTS',generatedAt:new Date().toISOString(),diagnostics:all,cohorts:{prospectiveAudited:prospective,legacyIncomplete:legacyDiag,counts:{all:rows.length,prospectiveAudited:audited.length,legacyIncomplete:legacy.length},interpretation:'Use PROSPECTIVE_AUDITED for post-fix model evaluation. LEGACY_INCOMPLETE remains immutable historical context and is never backfilled from current model state.'},policy:{diagnosticOnly:true,changesOfficialDecisionPolicy:false,forceSelectionBalance:false,noHistoricalAuditBackfill:true,primaryForwardCohort:'PROSPECTIVE_AUDITED',rule:'Never force HOME/DRAW/AWAY balance. Diagnose source calibration and selection concentration first; evaluate post-fix performance only on prospectively frozen audited records.'},recommendedNext:['ACCUMULATE_PROSPECTIVE_AUDITED_SAMPLE','COMPARE_CALIBRATION_AND_BRIER_PRE_POST_FIX','MONITOR_SELECTION_DISTRIBUTION_WITHOUT_FORCING_BALANCE','KEEP_EXPLORATORY_SHADOW_SEPARATE_FROM_OFFICIAL_VALIDATION']});
}
