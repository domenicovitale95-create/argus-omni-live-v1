...
const RUN_BUDGET_MS=105000;
const SETTLEMENT_RESERVE_MS=18000;
...
function endOfDay(c){return c.hour===23&&c.minute>=55}
function daytimeFastReconcile(c){return c.hour>=6&&c.hour<=23&&!endOfDay(c)}
...
async function fetchJson(url,options={},timeoutMs=REQUEST_TIMEOUT_MS){
 ...
 try{
  const r=await fetch(url,{...options,signal:controller.signal}),j=await r.json().catch(()=>({}));
  return{ok:r.ok&&j?.ok!==false,status:r.status,data:j};
 }catch(error){
  return{ok:false,status:0,data:{error:error?.name==='AbortError'?'REQUEST_TIMEOUT_'+timeoutMs+'MS':String(error?.message||error)}};
 }finally{clearTimeout(timer)}
}
async function call(base,mode,date,cycleAfter){const q=new URLSearchParams({mode});if(date)q.set('date',date);if(cycleAfter)q.set('cycleAfter',cycleAfter);return fetchJson(`${base}/api/prediction-ledger?${q.toString()}`,{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:'{}',cache:'no-store'},REQUEST_TIMEOUT_MS)}
async function callAutopilot(base){return fetchJson(`${base}/api/autopilot`,{method:'GET',headers:authHeaders(),cache:'no-store'},AUTOPILOT_TIMEOUT_MS)}
...
export default async function handler(req,res){
 ...
 if(!storageReady())return res.status(503).json({version:'PREDICTION-LEDGER-CRON-9',ok:false,error:'Storage unavailable'});
 const base=baseUrl(req);if(!base)return res.status(500).json({error:'Host unavailable'});
 const clock=brussels(),startedAt=new Date().toISOString(),deadline=Date.now()+RUN_BUDGET_MS,before=await planState(),active=scheduledActive(clock);
 ...
 if(cycle.ok&&cycle.captureRequired){
   if(Date.now()>=deadline-SETTLEMENT_RESERVE_MS)capture={status:null,ok:false,blocked:true,skipped:true,reason:'RUN_BUDGET_PROTECTED'};
   else{const c=await call(base,'capture',null,startedAt);capture={status:c.status,ok:c.ok,blocked:false,skipped:false,...c.data};}
 }else if(!cycle.ok){
   capture={status:null,ok:false,blocked:true,skipped:true,reason:cycle.reason};
 }
 if(daytimeFastReconcile(clock))settlements.push(Date.now()<deadline-SETTLEMENT_RESERVE_MS?{reason:'DAYTIME_FAST_RECONCILE',date:clock.date,...await call(base,'settle',clock.date,null)}:{reason:'DAYTIME_FAST_RECONCILE',date:clock.date,ok:true,status:null,data:{skipped:true,reason:'RUN_BUDGET_PROTECTED'}});
 if(endOfDay(clock))settlements.push(Date.now()<deadline-SETTLEMENT_RESERVE_MS?{reason:'END_OF_DAY',date:clock.date,...await call(base,'settle',clock.date,null)}:{reason:'END_OF_DAY',date:clock.date,ok:true,status:null,data:{skipped:true,reason:'RUN_BUDGET_PROTECTED'}});
 if([0,2,6,8].includes(clock.hour)&&clock.minute>=55){const previous=addDays(clock.date,-1),reason=`LATE_MATCH_RETRY_${[0,2,6,8].indexOf(clock.hour)+1}`;settlements.push(Date.now()<deadline-SETTLEMENT_RESERVE_MS?{reason,date:previous,...await call(base,'settle',previous,null)}:{reason,date:previous,ok:true,status:null,data:{skipped:true,reason:'RUN_BUDGET_PROTECTED'}})}
 const ok=cycle.ok&&capture.ok&&settlements.every(x=>x.ok);
 const state={version:'PREDICTION-LEDGER-CRON-9',startedAt,completedAt:new Date().toISOString(),baseHost:new URL(base).host,ok,cycle,capture,settlements:settlements.map(x=>({reason:x.reason,date:x.date,status:x.status,result:x.data})),policy:{captureRequiresCurrentCentralBrainCycle:true,currentCycleMustBeGeneratedThisCron:true,centralBrainFinalAuthorityRequired:true,benignNoCycleSkip:true,captureOnlyOnNewDecisionCycle:true,failClosedOnAutopilotFailure:true,failClosedOnCycleMismatch:true,freshPlanReadsRequired:true,daytimeSettlement:true,daytimeSettlementCadenceMinutes:5,providerRecheckMinutes:30,settlementProviderEligibilityAfterKickoffMinutes:150,endOfDaySettlement:true,lateMatchRetriesBrusselsHours:[0,2,6,8],productionDomainPreferred:true,automaticWagering:false}};
