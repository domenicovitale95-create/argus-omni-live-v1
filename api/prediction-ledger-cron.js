...
export default async function handler(req,res){
 ...
 const state={version:'PREDICTION-LEDGER-CRON-9',startedAt,completedAt:new Date().toISOString(),baseHost:new URL(base).host,ok,cycle,capture,settlements:settlements.map(x=>({reason:x.reason,date:x.date,status:x.status,result:x.data})),policy:{captureRequiresCurrentCentralBrainCycle:true,currentCycleMustBeGeneratedThisCron:true,centralBrainFinalAuthorityRequired:true,benignNoCycleSkip:true,captureOnlyOnNewDecisionCycle:true,failClosedOnAutopilotFailure:true,failClosedOnCycleMismatch:true,freshPlanReadsRequired:true,daytimeSettlement:true,daytimeSettlementCadenceMinutes:5,providerRecheckMinutes:30,settlementProviderEligibilityAfterKickoffMinutes:150,endOfDaySettlement:true,lateMatchRetriesBrusselsHours:[0,2],productionDomainPreferred:true,automaticWagering:false}};
