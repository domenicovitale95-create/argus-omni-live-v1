...
async function planState(){if(!storageReady())return{generatedAt:null,centralBrainAppliedAt:null,finalAuthority:false,rows:0};const plan=await readJson(PLAN_PATH,{generatedAt:null,plan:[]});return{generatedAt:plan?.generatedAt||null,centralBrainAppliedAt:plan?.centralBrain?.appliedAt||null,finalAuthority:plan?.centralBrain?.finalAuthority===true,systemMode:plan?.centralBrain?.systemMode||null,rows:Array.isArray(plan?.plan)?plan.plan.length:0}}
function currentCycleOk(state,startedAt){const g=new Date(state?.generatedAt||0).getTime(),a=new Date(state?.centralBrainAppliedAt||0).getTime(),s=new Date(startedAt||0).getTime();return Boolean(state?.finalAuthority&&Number.isFinite(g)&&g>0&&Number.isFinite(a)&&a>0&&Number.isFinite(s)&&s>0&&a>=s&&a>=g)}
export default async function handler(req,res){
 res.setHeader('Cache-Control','no-store');
 if(req.method!=='GET')return res.status(405).json({error:'Method not allowed'});
 if(!authorized(req))return res.status(401).json({error:'Unauthorized'});
 if(!storageReady())return res.status(503).json({version:'PREDICTION-LEDGER-CRON-5',ok:false,error:'Storage unavailable'});
 ...
 let cycle={required:active,attempted:false,ok:!active,reason:active?'CURRENT_CYCLE_REQUIRED':'OUTSIDE_AUTOPILOT_WINDOW',before,after:before,autopilot:null};
 if(active){
   const run=await callAutopilot(base),after=await planState(),attested=currentCycleOk(after,startedAt);
   cycle={required:true,attempted:true,ok:Boolean(run.ok&&attested),reason:run.ok?(attested?'CURRENT_CYCLE_ATTESTED':'CURRENT_CYCLE_ATTESTATION_FAILED'):'AUTOPILOT_FAILED',before,after,autopilot:{httpStatus:run.status,ok:run.ok,skipped:Boolean(run.data?.skipped),reason:run.data?.reason||null,error:run.data?.error||null,elapsedMs:run.data?.elapsedMs??null,centralBrain:run.data?.centralBrain||null}};
 }
 let capture={status:null,ok:false,blocked:true,reason:cycle.reason},settlements=[];
 if(cycle.ok){
   const c=await call(base,'capture',null,active?startedAt:null);capture={status:c.status,ok:c.ok,blocked:false,...c.data};
 }
 ...
 const state={version:'PREDICTION-LEDGER-CRON-5',startedAt,completedAt:new Date().toISOString(),baseHost:new URL(base).host,ok,cycle,capture,settlements:settlements.map(x=>({reason:x.reason,date:x.date,status:x.status,result:x.data})),policy:{captureRequiresCurrentCentralBrainCycle:true,currentCycleMustBeGeneratedThisCron:true,centralBrainFinalAuthorityRequired:true,failClosedOnAutopilotFailure:true,failClosedOnCycleMismatch:true,endOfDaySettlement:true,lateMatchRetriesBrusselsHours:[0,2],productionDomainPreferred:true,automaticWagering:false}};
