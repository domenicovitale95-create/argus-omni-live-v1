...
export default async function handler(req,res){
 ...
 if(!storageReady())return res.status(503).json({version:'PREDICTION-LEDGER-CRON-6',ok:false,error:'Storage unavailable'});
 ...
 const state={version:'PREDICTION-LEDGER-CRON-6',startedAt,completedAt:new Date().toISOString(),baseHost:new URL(base).host,ok,cycle,capture,settlements:settlements.map(x=>({reason:x.reason,date:x.date,status:x.status,result:x.data})),policy:{captureRequiresCurrentCentralBrainCycle:true,currentCycleMustBeGeneratedThisCron:true,centralBrainFinalAuthorityRequired:true,benignNoCycleSkip:true,captureOnlyOnNewDecisionCycle:true,failClosedOnAutopilotFailure:true,failClosedOnCycleMismatch:true,endOfDaySettlement:true,lateMatchRetriesBrusselsHours:[0,2],productionDomainPreferred:true,automaticWagering:false}};
