...
export default async function handler(req,res){
 ...
 if(daytimeFastReconcile(clock))settlements.push({reason:'DAYTIME_FAST_RECONCILE',date:clock.date,...await call(base,'settle',clock.date,null)});
 if(endOfDay(clock))settlements.push({reason:'END_OF_DAY',date:clock.date,...await call(base,'settle',clock.date,null)});
 if([0,2,6,8].includes(clock.hour)&&clock.minute>=55){const previous=addDays(clock.date,-1);settlements.push({reason:`LATE_MATCH_RETRY_${[0,2,6,8].indexOf(clock.hour)+1}`,date:previous,...await call(base,'settle',previous,null)})}
