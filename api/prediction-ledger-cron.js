...
async function call(base,mode,date){const r=await fetch(`${base}/api/prediction-ledger?mode=${mode}${date?`&date=${date}`:''}`,{method:'POST',headers:authHeaders({'Content-Type':'application/json'}),body:'{}',cache:'no-store'});const j=await r.json().catch(()=>({}));return{ok:r.ok,status:r.status,data:j}}
async function callAutopilot(base){const r=await fetch(`${base}/api/autopilot`,{method:'GET',headers:authHeaders(),cache:'no-store'});const j=await r.json().catch(()=>({}));return{ok:r.ok,status:r.status,data:j}}
...
export default async function handler(req,res){
 ...
 if(!storageReady())return res.status(503).json({version:'PREDICTION-LEDGER-CRON-4',ok:false,error:'Storage unavailable'});
 ...
 const state={version:'PREDICTION-LEDGER-CRON-4',startedAt,completedAt:new Date().toISOString(),baseHost:new URL(base).host,ok:capture.ok&&settlements.every(x=>x.ok),recovery,capture:{status:capture.status,...capture.data},settlements:settlements.map(x=>({reason:x.reason,date:x.date,status:x.status,result:x.data})),policy:{captureEveryCron:true,heartbeatPersistenceRequired:true,stalePlanSelfHealing:true,staleRecoveryMinutes:STALE_RECOVERY_MINUTES,recoveryOnlyDuringAutopilotWindow:true,endOfDaySettlement:true,lateMatchRetriesBrusselsHours:[0,2],productionDomainPreferred:true,automaticWagering:false}};
 try{
   await writeJson('argus/health/prediction-ledger-cron.json',state);
 }catch(error){
   return res.status(503).json({...state,ok:false,healthPersisted:false,error:`HEALTH_PERSIST_FAILED: ${error?.message||'unknown'}`});
 }
 return res.status(200).json({...state,healthPersisted:true,clock});
}
