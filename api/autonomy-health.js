...
const POLICY={
  ...
  globalWageringStatusReason:'THIS_HEALTH_ENDPOINT_DOES_NOT_OWN_OR_OBSERVE_A_REAL_WAGER_EXECUTOR',
...
export default async function handler(req,res){
  ...
  if(!storageReady())return res.status(503).json({version:'AUTONOMY-HEALTH-4',status:'CRITICAL',error:'Storage unavailable'});
  const state=await readJsonFresh(STATE_PATH,null);
  if(!state)return res.status(200).json({version:'AUTONOMY-HEALTH-4',status:'WAITING',heartbeatAgeMinutes:null,reason:'SUPERVISOR_HAS_NOT_RUN_YET',policy:POLICY});
  ...
  return res.status(200).json({
    ...
    policy:{...POLICY,cronExpectedEveryMinutes:EXPECTED_MINUTES,githubWatchdogEveryMinutes:30,githubRecoveryAfterMinutes:18,lateAfterMinutes:LATE_AFTER_MINUTES,staleAfterMinutes:STALE_AFTER_MINUTES,schedulerJitterTolerated:true,runsWithoutChat:true,consistentStateRead:true}
  });
}
