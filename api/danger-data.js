...
function nHeader(headers,name){const raw=headers.get(name);if(raw==null||raw==='')return null;const v=Number(raw);return Number.isFinite(v)?v:null}
let quota={dailyLimit:null,dailyRemaining:null,minuteLimit:null,minuteRemaining:null};
function reserve(){if(!quota.dailyLimit)return MIN_DAILY_RESERVE;return Math.max(MIN_DAILY_RESERVE,Math.min(MAX_DAILY_RESERVE,Math.ceil(quota.dailyLimit*.02)))}
...
function summarize(fixtures,teamId){
  ...
  let wins=0,draws=0,losses=0,gf=0,ga=0,homeGames=0,homePts=0,awayGames=0,awayPts=0,cleanSheets=0,failedToScore=0,btts=0,over25=0;const results=[];
  for(const f of rows){const home=Number(f.teams?.home?.id)===Number(teamId),away=Number(f.teams?.away?.id)===Number(teamId);if(!home&&!away)continue;const scored=Number(home?f.goals?.home:f.goals?.away)||0,conceded=Number(home?f.goals?.away:f.goals?.home)||0,pts=scored>conceded?3:scored===conceded?1:0;if(pts===3)wins++;else if(pts===1)draws++;else losses++;gf+=scored;ga+=conceded;if(conceded===0)cleanSheets++;if(scored===0)failedToScore++;if(scored>0&&conceded>0)btts++;if(scored+conceded>2)over25++;if(home){homeGames++;homePts+=pts}else{awayGames++;awayPts+=pts}results.push({timestamp:f.fixture?.timestamp||null,gf:scored,ga:conceded,points:pts});}
  const recent=results.slice(-5),round=v=>Number(v.toFixed(3)),count=results.length;
  return {matches:count,wins,draws,losses,pointsPerGame:count?round((wins*3+draws)/count):0,goalsForPerGame:count?round(gf/count):0,goalsAgainstPerGame:count?round(ga/count):0,winRate:count?round(wins/count):0,cleanSheetRate:count?round(cleanSheets/count):0,failedToScoreRate:count?round(failedToScore/count):0,bttsRate:count?round(btts/count):0,over25Rate:count?round(over25/count):0,homeGames,homePPG:homeGames?round(homePts/homeGames):null,awayGames,awayPPG:awayGames?round(awayPts/awayGames):null,last5PPG:recent.length?round(recent.reduce((s,r)=>s+r.points,0)/recent.length):0,windowDays:HISTORY_DAYS};
}
...
export default async function handler(req,res){
  try{
    ...
    const cachePath='argus/data/team-history-90d.json';let aggregate=storageReady()?await readJson(cachePath,{teams:{}}):{teams:{}};aggregate.teams||={};const cache=aggregate.teams;
    const cutoff=Date.now()-CACHE_HOURS*3600000,missing=teamIds.filter(id=>!cache[String(id)]||new Date(cache[String(id)].savedAt||0).getTime()<cutoff);
    const dailyAvailable=quota.dailyRemaining==null?MAX_BATCH_SIZE:Math.max(0,quota.dailyRemaining-reserve());
    const minuteAvailable=quota.minuteRemaining==null?MAX_BATCH_SIZE:Math.max(0,quota.minuteRemaining-MIN_MINUTE_RESERVE);
    const batchSize=Math.max(0,Math.min(MAX_BATCH_SIZE,dailyAvailable,minuteAvailable,missing.length));
    const candidates=missing.slice(0,batchSize);
    ...
    if(storageReady()&&loaded)await writeJson(cachePath,aggregate);
    const normalized=active.map(f=>normalizeFixture(f,cache)),usable=normalized.filter(m=>m.history90d.home?.matches>=3&&m.history90d.away?.matches>=3).length;
    const stillMissing=teamIds.filter(id=>!cache[String(id)]||!cache[String(id)].data||cache[String(id)].data.matches<3).length;
    res.setHeader('Cache-Control','no-store');
    return res.status(200).json({date,matches:normalized,meta:{fetchedAt:new Date().toISOString(),mode:'PRO-DYNAMIC',activeMatches:active.length,totalTeams:teamIds.length,loadedThisCall:loaded,batchSize,teamsWithCache:teamIds.length-stillMissing,teamsStillMissing:stillMissing,usableMatches:usable,hasMore:missing.length>candidates.length,storage:storageReady()?'shared-blob':'memory-only',quota:{...quota,dynamicReserve:reserve()}}});
