...
function potentiallyCaptureable(rec,now){const p=ms(rec?.publishedAt),k=ms(rec?.kickoff),odds=n(rec?.odds),age=p?(now-p)/60000:null;return Boolean(rec?.id&&rec?.fixtureId&&p&&k&&p<k&&now<k&&age>=0&&age<=MAX_LEDGER_CAPTURE_AGE_MIN&&odds>1&&rec?.integrity?.settlementSupported!==false)}
...
export default async function handler(req,res){
  ...
  const ambiguous=fresh.filter(rec=>!proof.appliedAtMs||ms(rec.publishedAt)<proof.appliedAtMs);
  if(fresh.length&&(!proof.ok||ambiguous.length))return res.status(200).json({version:'VIRTUAL-BANKROLL-4',ok:false,status:'BLOCKED',reason:!proof.ok?'CURRENT_CYCLE_ATTESTATION_FAILED':'AMBIGUOUS_PREVIOUS_CYCLE_LEDGER_RECORD',capturedOfficial:0,capturedLearning:0,settled:0,providerCalls:0,attestation:proof,freshLedgerRecords:fresh.length,ambiguousRecords:ambiguous.map(x=>({id:x.id,fixtureId:x.fixtureId,publishedAt:x.publishedAt})).slice(0,20),policy:{failClosed:true,noNewVirtualPositionOnCycleAmbiguity:true,settlementMayResumeWhenNoFreshAmbiguousEntryExists:true,automaticRealBetPlacement:false,noRealMoney:true}});
  ...
  return res.status(captureRes.statusCode).json({...body,version:'VIRTUAL-BANKROLL-4',attestation:proof,freshLedgerRecords:fresh.length,cycleAmbiguity:false,decisionCycleId:proof.decisionCycleId,policy:{...(body.policy||{}),failClosedOnCycleAmbiguity:true,currentCentralBrainCycleRequiredForNewEntries:true,automaticRealBetPlacement:false,noRealMoney:true}});
}
