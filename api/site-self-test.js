...
async function probe(base,path){
  ...
  try{
    const r=await fetch(`${base}${path}`,{cache:'no-store',headers:{Accept:path.startsWith('/api/')?'application/json':'text/html'}});
    return{path,ok:r.ok,status:r.status,ms:Date.now()-started,contentType:r.headers.get('content-type')||''};
  }catch(e){return{path,ok:false,status:0,ms:Date.now()-started,error:e.message}}
}
export default async function handler(req,res){
  ...
  const paths=['/','/daily-slip.html','/markets.html','/exact-scores.html','/training-dashboard.html','/compte-rendu-des-predictions.html','/argus-lab.html','/manifest.webmanifest','/sw.js','/api/site-health','/api/decision-scheduler'];
