...
async function settleFromProvider(state,now,nowMs){
 ...
 if(!overdue.length)return{settled:0,providerCalls:0,reason:'NO_OVERDUE_OPEN_BETS',dates:[]};
 const lastMs=new Date(state.settlementProbe?.lastProviderProbeAt||0).getTime();
 if(Number.isFinite(lastMs)&&lastMs>0&&nowMs-lastMs<RESULT_PROBE_COOLDOWN_MIN*60000)return{settled:0,providerCalls:0,reason:'PROVIDER_PROBE_COOLDOWN',dates:state.settlementProbe?.lastProviderDates||[]};
 if(await providerBlocked())return{settled:0,providerCalls:0,reason:'PROVIDER_BLOCKED_BY_QUOTA_GUARD',dates:[]};
 ...
 let settled=0,calls=0,error=null;
 state.settlementProbe.lastProviderProbeAt=now;
 state.settlementProbe.lastProviderDates=dates;
 ...
 for(const date of dates){
  ...
  for(const bet of overdue){
   ...
   if(!fixture)continue;
   const result=providerResult(fixture);
   if(result.status==='VOID'){if(applySettlement(state,bet,'VOID',null,now,'API_FOOTBALL_TRAINING_SETTLEMENT',result.providerStatus))settled++;continue}
   if(result.status!=='FINAL')continue;
   const outcome=selectionOutcome(bet.selection,result.score);
   if(!outcome)continue;
   if(applySettlement(state,bet,outcome,result.score,now,'API_FOOTBALL_TRAINING_SETTLEMENT',result.providerStatus))settled++;
  }
 }
 ...
 return{settled,providerCalls:calls,reason:error?'PROVIDER_ERROR':settled?'SETTLED_FROM_PROVIDER':'PROVIDER_CHECK_NO_FINAL_RESULT',dates,error};
}
function view(state,run={}){
 ...
 return{version:state.version,generatedAt:new Date().toISOString(),summary:{bankroll:state.bankroll,pnl:Number((state.bankroll-INITIAL_BANKROLL).toFixed(2)),tracked:rows.length,open:rows.filter(x=>x.status==='OPEN').length,overdueOpen:overdueOpen(state,Date.now()).length,settled:decided.length+voids,wins,losses,voids,hitRatePct:decided.length?Number((wins/decided.length*100).toFixed(1)):null,totalStaked:state.totalStaked},run,settlementProbe:state.settlementProbe,integrity:state.integrity,bets:visible};
}
