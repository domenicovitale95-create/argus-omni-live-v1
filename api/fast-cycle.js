...
export default async function handler(req,res){
  ...
  if(!storageReady())return res.status(503).json({version:'FAST-CYCLE-2',status:'CRITICAL',error:'Storage unavailable'});
  ...
  if(previous?.status==='RUNNING'&&ageMs(previous.startedAt)<ACTIVE_LOCK_MS){
    return res.status(200).json({version:'FAST-CYCLE-2',status:'SKIPPED_IN_PROGRESS',startedAt:previous.startedAt,runId:previous.runId||null,source:sourceOf(req),policy:{duplicateExecutionSuppressed:true}});
  }
  ...
  await writeJson(STATE,{version:'FAST-CYCLE-2',runId,source,status:'RUNNING',startedAt,completedAt:null,results:[],policy:{primaryScheduler:'VERCEL_CRON',backupScheduler:'GITHUB_CONDITIONAL',automaticRealWagering:false}});
  ...
  const state={
    version:'FAST-CYCLE-2',runId,source,startedAt,completedAt,
    ...
    policy:{primaryScheduler:'VERCEL_CRON',primaryCadenceMinutes:5,backupScheduler:'GITHUB_CONDITIONAL',backupHealthCheckMinutes:10,inProcessSubjobs:true,networkFallbackOnLocalException:true,providerCallsAddedByOrchestrator:0,subjobsPreserveOwnProviderPolicies:true,automaticRealWagering:false}
  };
