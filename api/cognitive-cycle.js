...
export default async function handler(req,res){
 ...
 try{
  ...
  const hypotheses=Array.isArray(hypothesisPacket.hypotheses)?hypothesisPacket.hypotheses:[];
  const active=hypotheses.map(h=>{const k=key(h),old=prevMap.get(k),occurrences=Number(old?.occurrences||0)+1,firstSeen=old?.firstSeen||now;return{key:k,...compactHypothesis(h),firstSeen,lastSeen:now,occurrences,attentionScore:weight(h.priority)+Math.min(20,Math.max(0,occurrences-1)*2)}}).sort((a,b)=>b.attentionScore-a.attentionScore);
  ...
  const snapshot={version:'COGNITIVE-CYCLE-1',generatedAt:now,mode:'SHADOW_READ_ONLY',productionAuthority:false,llmConnected:false,brief:{version:brief.version||null,summary:brief.summary||{},priorities:Array.isArray(brief.priorities)?brief.priorities:[]},hypotheses:{version:hypothesisPacket.version||null,count:hypotheses.length},memory:{active,recentlyResolved},attention:{top:active.slice(0,5),recurring:active.filter(x=>x.occurrences>=3).slice(0,8)},constraints:{mayChangeProduction:false,mayUnlockPrime:false,mayChangeWeights:false,mayChangePolicies:false,mayChangeStake:false,governanceVeto:true},storage:{ready:storageReady(),latestPath:LATEST},elapsedMs:Date.now()-started};
  if(storageReady()){
