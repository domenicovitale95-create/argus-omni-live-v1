...
function dateList(){const out=[];const end=new Date(Date.now()-86400000);for(let i=DAYS-1;i>=0;i--)out.push(ymd(new Date(end.getTime()-i*86400000)));return out}
...
export default async function handler(req,res){
 ...
 const dates=dateList(),archive=await readJson(ARCHIVE,{version:'HISTORICAL-DECADE-ARCHIVE-1',fixtures:{},dates:{}}),state=await readJson(STATE,{cursor:0,complete:false});archive.fixtures||={};archive.dates||={};let cursor=Math.max(0,Number(state.cursor)||0),processed=0,newFixtures=0,lastQuota=null,errors=[];
 const requested=Math.max(1,Math.min(MAX_BATCH,Number(req.query?.dates)||DEFAULT_BATCH));
 while(cursor<dates.length&&processed<requested){const date=dates[cursor];if(archive.dates[date]?.complete){cursor++;continue}try{const out=await fetchDate(date);lastQuota=out.quota;for(const row of out.rows){const k=String(row.fixtureId);if(!archive.fixtures[k])newFixtures++;archive.fixtures[k]=row}archive.dates[date]={complete:true,fixtures:out.rows.length,savedAt:new Date().toISOString()};processed++;cursor++;const r=reserve(lastQuota);if(lastQuota?.dailyRemaining!=null&&lastQuota.dailyRemaining<=r)break;if(lastQuota?.minuteRemaining!=null&&lastQuota.minuteRemaining<=3)break}catch(e){errors.push({date,error:e.message});break}}
 state.cursor=cursor;state.complete=cursor>=dates.length;state.lastRun=new Date().toISOString();state.lastQuota=lastQuota;archive.windowDays=DAYS;archive.windowStart=dates[0];archive.windowEnd=dates.at(-1);archive.fixtureCount=Object.keys(archive.fixtures).length;archive.completedDates=Object.values(archive.dates).filter(x=>x?.complete).length;archive.updatedAt=state.lastRun;await writeJson(ARCHIVE,archive);await writeJson(STATE,state);
 return res.status(errors.length?207:200).json({ok:errors.length===0,version:'HISTORICAL-DECADE-BACKFILL-1',windowDays:DAYS,windowStart:archive.windowStart,windowEnd:archive.windowEnd,processedDates:processed,completedDates:archive.completedDates,totalDates:dates.length,newFixtures,totalFixtures:archive.fixtureCount,cursor,complete:state.complete,quota:lastQuota,reserve:reserve(lastQuota),errors,policy:{globalByDate:true,tenYears:true,exactScoresStored:true,persistentArchive:true,quotaAware:true,noRepeatedFetchForCompletedDates:true,historicalOddsNotFabricated:true}})
}
