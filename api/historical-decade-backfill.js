...
export default async function handler(req,res){
  ...
  console.log('[historical-decade-backfill] archive_meta',JSON.stringify({pathname:archiveMeta?.pathname||ARCHIVE,sizeBytes:archiveSizeBytes,uploadedAt:archiveMeta?.uploadedAt||null}));
  if(archiveSizeBytes!=null&&archiveSizeBytes>MAX_MONOLITH_BYTES){
    return res.status(200).json({ok:true,version:'HISTORICAL-DECADE-BACKFILL-3',status:'PAUSED_MEMORY_GUARD',generatedAt:new Date().toISOString(),archiveSizeBytes,maxMonolithBytes:MAX_MONOLITH_BYTES,processedDates:0,providerCalls:0,migrationRequired:true,recommendation:'Migrate the historical archive to year/month shards before resuming decade backfill.',policy:{failClosed:true,noOOMRiskAccepted:true,noProviderQuotaSpend:true,automaticBackfillPaused:true}});
  }

  const dates=dateList(),archive=await readJson(ARCHIVE,{version:'HISTORICAL-DECADE-ARCHIVE-2',fixtures:{},dates:{}}),state=await readJson(STATE,{complete:false});
  archive.fixtures||={};archive.dates||={};let processed=0,newFixtures=0,lastQuota=null,errors=[];
  ...
  return res.status(errors.length?207:200).json({ok:errors.length===0,version:'HISTORICAL-DECADE-BACKFILL-3',status:errors.length?'PARTIAL':'OK',strategy:'RECENT_FIRST',windowDays:DAYS,windowStart:archive.windowStart,windowEnd:archive.windowEnd,processedDates:processed,requestedDates:requested,completedDates,totalDates:dates.length,recentCompleteDays:recentCompletePrefix,newFixtures,totalFixtures:archive.fixtureCount,complete:state.complete,quota:lastQuota,reserve:reserve(lastQuota),archiveSizeBytesBeforeRun:archiveSizeBytes,errors,policy:{globalByDate:true,tenYears:true,recentDatesFirst:true,exactScoresStored:true,persistentArchive:true,quotaAware:true,noRepeatedFetchForCompletedDates:true,historicalOddsNotFabricated:true,memoryGuard:true,maxBatch:MAX_BATCH}})
}
