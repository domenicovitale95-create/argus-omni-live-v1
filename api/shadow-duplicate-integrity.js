...
export default async function handler(req,res){
  ...
  try{
    const blobs=await listJson('argus/shadow/',240),books=await readManyJson(blobs),rawId=String(req.query?.fixtureId||'').trim(),onlyIds=rawId?[rawId]:null,result=inspectShadowFixtureDuplicates(books,{onlyIds,limit:rawId?1:20});
