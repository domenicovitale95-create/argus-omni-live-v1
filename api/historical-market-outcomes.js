...
export default async function handler(req,res){
  ...
  const a=await readJson(IN,null);if(!a)return res.status(200).json({version:'HISTORICAL-MARKET-OUTCOMES-2',status:'WAITING_FOR_DECADE_ARCHIVE',records:0});
  const rows=Object.values(a.fixtures||{}).map(derive).filter(Boolean),summary={records:rows.length,exactScores:{},matchWinner:{},btts:{},goalBands:{},total25:{},homeTotal05:{},awayTotal05:{}};for(const r of rows){inc(summary.exactScores,r.exactScore);inc(summary.matchWinner,r.matchWinner);inc(summary.btts,r.btts);inc(summary.goalBands,r.goalBands);inc(summary.total25,r.totals['2.5']);inc(summary.homeTotal05,r.homeTotals['0.5']);inc(summary.awayTotal05,r.awayTotals['0.5'])}
  const report={version:'HISTORICAL-MARKET-OUTCOMES-2',generatedAt:new Date().toISOString(),sourceVersion:a.version||null,windowStart:a.windowStart||null,windowEnd:a.windowEnd||null,archiveSizeBytes,records:rows.length,marketFamilies:['1X2','DOUBLE_CHANCE','DRAW_NO_BET','BTTS','CLEAN_SHEET','WIN_TO_NIL','TOTAL_GOALS','TEAM_TOTALS','GOAL_BANDS','ODD_EVEN','WIN_MARGIN','WINNER_BTTS','WINNER_TOTAL_2.5','EXACT_TOTAL_GOALS','EXACT_SCORE','ASIAN_HANDICAP_RESULT'],summary,sample:rows.slice(-500),policy:{historicalResearchOnly:true,derivedFromVerifiedFinalScore:true,noHistoricalOdds:true,noHistoricalROI:true,cornersCardsRequireStatisticsEnrichment:true,memoryGuard:true,maxMonolithBytes:MAX_MONOLITH_BYTES}};await writeJson(OUT,report);return res.status(200).json(report)
}
