...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');if(!storageReady())return res.status(503).json({error:'Decision scheduler storage unavailable'});if(req.method==='GET')return res.status(200).json(await readJson(STATE_PATH,{version:'DECISION-SCHEDULER-2',generatedAt:null,plan:[],summary:{}}));if(req.method!=='POST')return res.status(405).json({error:'Method not allowed'});const matches=Array.isArray(req.body?.matches)?req.body.matches:[],meta=req.body?.meta||{},events=Array.isArray(req.body?.events)?req.body.events:[],mode=quotaMode(meta);let plan=matches.map(m=>({fixtureId:m.id,competition:m.competition,home:m.home,away:m.away,kickoff:m.kickoff,status:m.status,isLive:Boolean(m.isLive),minute:m.minute??null,...tier(m)}));plan=applyQuotaCadence(plan,mode);plan=applyEventBoost(plan,events).sort((a,b)=>b.score-a.score);const summary={total:plan.length,livePriority:plan.filter(x=>x.tier==='LIVE_PRIORITY').length,valueCandidates:plan.filter(x=>x.tier==='VALUE_CANDIDATE').length,watch:plan.filter(x=>x.tier==='WATCH').length,base:plan.filter(x=>x.tier==='BASE').length,archive:plan.filter(x=>x.tier==='ARCHIVE').length,eventDriven:plan.filter(x=>x.eventDriven).length,quotaMode:mode,nextUrgentFixture:plan.find(x=>['LIVE_PRIORITY','VALUE_CANDIDATE','WATCH'].includes(x.tier))?.fixtureId||null};const state={version:'DECISION-SCHEDULER-2',generatedAt:new Date().toISOString(),policy:{rule:'Prioritization controls monitoring cadence only. Events may accelerate monitoring but never create or upgrade a betting verdict.',minimumCadenceMinutes:5,quotaAware:true,eventDriven:true},summary,plan:plan.slice(0,500)};await writeJson(STATE_PATH,state);return res.status(200).json(state)}