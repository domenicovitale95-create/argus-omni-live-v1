...
const QUOTA='argus/autopilot/quota-efficiency.json';
...
function effectiveResource(resource={},quota={}){
  const quotaMode=String(quota?.budget?.mode||'UNKNOWN').toUpperCase();
  const resourceMode=String(resource.mode||resource.quotaMode||'UNKNOWN').toUpperCase();
  const mode=quotaMode&&quotaMode!=='UNKNOWN'?quotaMode:resourceMode;
  return{...resource,mode,quotaMode:mode,quotaTelemetry:quota?.telemetry||null,quotaBudget:quota?.budget||null,quotaGeneratedAt:quota?.generatedAt||null};
}
...
function prematchCandidate(row,resource){
  ...
  const offlineBoost=constrained(resource)?8:0;
  const score=clamp((Number(row.score)||20)*.65+urgency+unresolved+offlineBoost);
  return{population:'FUTURE_PREMATCH',kind:'REVIEW_PREMATCH',fixtureId:row.fixtureId,competition:row.competition||null,kickoff:row.kickoff||null,priorityScore:Number(score.toFixed(1)),providerDependent:false,reason:`existing decision score ${Number(row.score)||0}; kickoff ${ko}m; verdict ${row.finalVerdict||'UNSET'}${offlineBoost?'; cached prematch boosted while provider constrained':''}`};
}
...
export default async function handler(req,res){
  ...
  const [planDoc,rawResource,quota,skill,walk,ledgerBlobs]=await Promise.all([
  ...
  const resource=effectiveResource(rawResource,quota),books=await readManyJson(ledgerBlobs),plan=Array.isArray(planDoc?.plan)?planDoc.plan:[];
  ...
  const candidates=[];
  for(const row of future.slice().sort((a,b)=>(Number(b.score)||0)-(Number(a.score)||0)).slice(0,12))candidates.push(prematchCandidate(row,resource));
  ...
  return res.status(200).json({
    version:'MATCH-UNIVERSE-SCHEDULER-2',generatedAt:new Date().toISOString(),readOnly:true,providerCalls:0,
    ...
    resource:{mode:resource.mode||null,quotaMode:resource.quotaMode||null,providerConstrained:constrained(resource),quotaTelemetry:resource.quotaTelemetry,quotaBudget:resource.quotaBudget,quotaGeneratedAt:resource.quotaGeneratedAt},
    ...
    scoring:{type:'transparent heuristic v2',principle:'Expected learning/decision value adjusted by urgency, stored-evidence freshness and quota/resource cost. Existing ARGUS scores are reused; missing evidence is not invented.'},
    reuse:{quotaEfficiencySnapshot:true,decisionScheduler:true,predictionLedger:true,historicalWalkForward:true,memoryGuardAware:true},
