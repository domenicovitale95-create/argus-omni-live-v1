...
export default async function handler(req,res){
  ...
  if(!storageReady())return res.status(503).json({version:'HISTORICAL-ARCHIVE-HEALTH-4',status:'DEGRADED',error:'Storage unavailable'});
  ...
  if(!meta)return res.status(200).json({version:'HISTORICAL-ARCHIVE-HEALTH-4',status:'NO_ARCHIVE',providerCalls:false,writes:false});
  ...
  const firstOverlayGaps=dates.filter(d=>!overlay[d]?.complete).slice(0,30),months=Object.entries(shardIndex?.months||{}).filter(([,v])=>v?.complete),migratedFixtureCount=months.reduce((s,[,v])=>s+(Number(v.fixtureCount)||0),0),migrationComplete=Boolean(shardIndex?.migrationComplete),migrationStatus=migrationComplete?'COMPLETE':months.length?'IN_PROGRESS':'NOT_STARTED';
  const status=migrationComplete?'SHARDED_ARCHIVE_HEALTHY':overlayRecentCompleteDays>=30?'RECENT_CONTINUITY_HEALTHY':memoryGuardTriggered?'SHARD_MIGRATION_REQUIRED':'LEGACY_ARCHIVE_AVAILABLE';
  return res.status(200).json({version:'HISTORICAL-ARCHIVE-HEALTH-4',generatedAt:new Date().toISOString(),status,archiveSizeBytes,memoryGuardBytes:MEMORY_GUARD_BYTES,memoryGuardTriggered,legacyArchiveRead:false,windowDays:DAYS,legacyState:{complete:Boolean(state?.complete),lastRun:state?.lastRun||null,recentCompleteDays:Number(state?.recentCompleteDays)||0},shards:{status:migrationStatus,migrationComplete,months:months.length,fixtureCount:migratedFixtureCount,windowStart:shardIndex?.windowStart||null,windowEnd:shardIndex?.windowEnd||null,updatedAt:shardIndex?.updatedAt||null,lastRun:shardIndex?.lastRun||null},recentOverlay:{version:recentIndex?.version||null,migrationFloor:recentIndex?.migrationFloor||null,requiredThrough:recentIndex?.requiredThrough||null,updatedAt:recentIndex?.updatedAt||null,months:Object.keys(recentIndex?.months||{}).length,completeDates:overlayCompleteDates,recentCompleteDays:overlayRecentCompleteDays,firstGaps:firstOverlayGaps},policy:{readOnly:true,providerCalls:false,writes:false,diagnosticOnly:true,largeLegacyBlobNeverParsed:true,doesNotRelaxTrendIntegrity:true,memoryGuardObserved:true,monthlyShardMigration:true}})
}
