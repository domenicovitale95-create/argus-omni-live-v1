...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');if(req.method!=='POST')return res.status(405).json({error:'Method not allowed'});const matches=Array.isArray(req.body?.matches)?req.body.matches:[],gates=Array.isArray(req.body?.preKickoffGates)?req.body.preKickoffGates:[],gm=new Map(gates.map(g=>[String(g.fixtureId),g]));let attr=null;if(storageReady())try{attr=await readJson('argus/learning/error-attribution.json',null)}catch(_){}const decisions={};for(const m of matches)decisions[String(m.id)]={fixtureId:m.id,home:m.home,away:m.away,kickoff:m.kickoff,...verdictFor(m,gm.get(String(m.id)),attr)};const vals=Object.values(decisions);return res.status(200).json({ok:true,version:'BET-ELIGIBILITY-4',generatedAt:new Date().toISOString(),summary:{total:vals.length,prime:vals.filter(x=>x.verdict==='PRIME').length,value:vals.filter(x=>x.verdict==='VALUE').length,watch:vals.filter(x=>x.verdict==='WATCH').length,noBet:vals.filter(x=>x.verdict==='NO BET').length,eligible:vals.filter(x=>x.eligible).length,attributionPenalized:vals.filter(x=>n(x.attributionPenalty,0)>0).length,preKickoffBlocked:vals.filter(x=>x.gateStatus==='BLOCKED').length},policy:{primeEdgeMinPct:6,primeConfidenceMin:68,primeQualityMin:70,valueEdgeMinPct:3.5,valueConfidenceMin:58,valueQualityMin:60,watchEdgeMinPct:2,primeMayBeCreatedOnlyHere:true,gateCanBlock:true,blockedGateIsHardVeto:true,confidencePenaltiesApplied:true,errorAttributionApplied:true,maxAttributionPenalty:8,positiveAttributionCannotPromote:true,marketRegimeApplied:true,rule:'Final verdict is evidence-based and downgrade-first. A BLOCKED pre-kickoff gate is always NO BET and no upstream module may bypass eligibility governance.'},decisions})}
