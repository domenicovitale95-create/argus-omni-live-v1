...
function verdictFor(m,gate,attr){
 const c=candidate(m),q=quality(m),qSource=qualitySource(m),ap=attrPenalty(m,c,attr),conf=confidence(m,c,ap.penalty),regime=String(m?.marketRegime?.regime||'UNKNOWN'),movement=String(m?.marketRegime?.directionalStatus||'NEUTRAL'),issues=[...ap.reasons],positive=[];
