...
async function json(url,options={}){const r=await fetch(url,{...options,headers:{Accept:'application/json',...(options.headers||{})}});const j=await r.json().catch(()=>({}));if(!r.ok)throw new Error(j.error||`HTTP ${r.status}`);return j}
export default async function handler(req,res){
  ...
  try{
    const live=await json(`${base}/api/live`),matches=(live.matches||[]).filter(m=>!m.isFinished);
    if(!matches.length)return res.status(200).json({ok:true,skipped:true,reason:'NO_ACTIVE_FIXTURES'});
