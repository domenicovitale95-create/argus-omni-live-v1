...
export function completeness(zone){
  const required=[
  ...
  const present=required.filter(k=>Number.isFinite(Number(zone.scores?.[k]))).length;
...
export function calculateUrbanUpside(zone){
  const s={...(zone.scores||{})};
  ...
  const missing=Math.max(0,100-completeness(zone));
...
export function derivedIndicators(zone){
  const s={...(zone.scores||{})};
  s.executionCertainty=Number.isFinite(Number(s.executionCertainty))
    ?n(s.executionCertainty)
    :executionFromProjects(zone.projects||[]);
  ...
  return {
    urbanUpside:calculateUrbanUpside(zone),
    currentValue,
    futureCatalysts,
    executionConfidence:clamp(s.executionCertainty),
    risk:clamp(n(s.riskScore,0)),
    dataCompleteness:completeness(zone)
  };
}

export function rankZones(zones=[]){
  return zones.map(z=>({...z,derived:derivedIndicators(z)}))
    .sort((a,b)=>b.derived.urbanUpside-a.derived.urbanUpside);
}

export function topOpportunityBuckets(zones=[]){
  const ranked=rankZones(zones).filter(z=>z.status!=='REFERENCE_ONLY');
