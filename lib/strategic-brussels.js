...
export function deriveMarketScores(zone,benchmarks={}){
  ...
  if(!isNum(s.valueScore)&&Number.isFinite(p)&&Number.isFinite(region)&&region>0){
    const ratio=p/region;
    s.valueScore=clamp(50+(1-ratio)*115);
  }
  if(!isNum(s.affordability)&&Number.isFinite(p)&&Number.isFinite(region)&&region>0){
    const ratio=p/region;
    s.affordability=clamp(50+(1-ratio)*100);
  }
  const pm=zone.market?.priceMomentum||{};
  if(!isNum(s.priceMomentum)){
    const vals=[
      isNum(pm.oneYear)?n(pm.oneYear)*.45:null,
      isNum(pm.threeYear)?n(pm.threeYear)/3*.35:null,
      isNum(pm.fiveYear)?n(pm.fiveYear)/5*.20:null
    ].filter(v=>v!=null);
    if(vals.length){
