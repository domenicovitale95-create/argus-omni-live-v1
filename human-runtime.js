...
(() => {
  ...
  function addNavLink(nav,href,label,beforeSelector){
  ...
  function ensurePrimaryNav(){
    ...
    if(!nav)return;
    addNavLink(nav,'daily-slip.html','Daily Slip','a[href="exact-scores.html"]');
    addNavLink(nav,'markets.html','Markets','a[href="training-dashboard.html"]');
    addNavLink(nav,'training-dashboard.html','Training','.system-link');
    addNavLink(nav,'compte-rendu-des-predictions.html','Performance','.system-link');
    addNavLink(nav,'evolution-dashboard.html','Evolution','.system-link');
  }

  function ensureResponsiveNav(){
    if(document.getElementById('argus-runtime-nav-style')) return;
    const style=document.createElement('style');
    style.id='argus-runtime-nav-style';
    style.textContent=`
      @media (max-width: 900px){
        .topbar{display:flex!important;flex-direction:column!important;align-items:stretch!important;gap:10px!important;padding:12px 0 9px!important;min-height:auto!important}
        .topbar>.brand{align-self:flex-start!important}
        .top-status{display:flex!important;width:100%!important;max-width:100%!important;gap:7px!important;overflow-x:auto!important;overflow-y:hidden!important;flex-wrap:nowrap!important;padding:2px 0 7px!important;-webkit-overflow-scrolling:touch!important;scrollbar-width:none!important}
        .top-status::-webkit-scrollbar{display:none!important}
        .top-status .nav-link{display:inline-flex!important;align-items:center!important;justify-content:center!important;flex:0 0 auto!important;white-space:nowrap!important;padding:9px 11px!important;font-size:8px!important}
        .top-status .system-status{display:flex!important;flex:0 0 auto!important;min-width:24px!important}
        .top-status .safe-chip{flex:0 0 auto!important}
      }
      @media (max-width: 520px){
        .top-status .nav-link{padding:8px 10px!important;font-size:7.5px!important}
        .top-status .system-status{display:none!important}
      }
    `;
