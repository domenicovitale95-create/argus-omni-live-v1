(() => {
  ...
  const fmtEur=v=>new Intl.NumberFormat('en-GB',{style:'currency',currency:'EUR',maximumFractionDigits:0}).format(Number(v)||0);
  ...
  function statusIt(v){return v||'—'}
  function translateEvidence(v){return String(v??'')}
  ...
  function riskGovernor(data){
    ...
    if(!data) reasons.push('No verified market snapshot has been loaded.');
    if(coverage<60) reasons.push('Verified data coverage is below 60%.');
    if(global==null) reasons.push('The global market score is unavailable.');
    if(!num(best.score)) reasons.push('No scored opportunity has passed the data checks.');
    if(String(risk.severity||'').toUpperCase()==='HIGH' && (num(best.score)||0)<80) reasons.push('High risk without exceptionally strong evidence.');

    const blocked=reasons.length>0;
    return {
      shadow:blocked?'WAIT / CASH':'VIRTUAL ALLOCATION ALLOWED',
      real:'BLOCKED',
      blocked,
      coverage,
      reasons:blocked?reasons:['Verified data is sufficient for a virtual-money simulation.']
    };
  }
  ...
  function scenarioLens(data){
    ...
    if(score==null) return {bull:null,base:null,bear:null,label:'INSUFFICIENT DATA'};
  ...
  function debate(data){
    ...
    if(!bull.length) bull.push('No verified bull case has passed the evidence checks.');
  ...
  function buildVirtualPortfolio(capital,profile,data,config){
    ...
    if(governor.blocked){
      return {
        ...
        governorAtEntry:governor.shadow,note:'The risk check kept the virtual portfolio entirely in cash.'
      };
    }
    ...
    if(!picks.length){
      return {
        version:2,createdAt:now,profile,startingCapital:capital,cash:capital,positions:[],
        benchmarkEntry:globalPrice,benchmarkEntryDate:data?.market?.global_equity?.date||null,
        governorAtEntry:'WAIT / CASH',note:'No candidate passed score, proxy price and data-quality checks together.'
      };
    }
    ...
    return {
      ...
      benchmarkEntry:globalPrice,benchmarkEntryDate:data?.market?.global_equity?.date||null,
      governorAtEntry:governor.shadow,note:'Simulated portfolio. This module cannot create real orders.'
    };
  }
  ...
  function renderIntelligence(){
    ...
    if(packet){
      ...
      packet.innerHTML=[
        ...
        ['EVIDENCE',statusIt(q.state||'WEAK')+' · '+(q.coverage_pct??0)+'%'],
        ['DEBATE',statusIt(d.action||'WAIT')],
        ['RISK CHECK',gov.shadow]
  ...
  function renderVirtual(){
    ...
    if(!state){
      root.innerHTML='<div class="empty">No virtual portfolio yet. Choose capital and profile, then start ARGUS.</div>';
    ...
    if(!(state.positions||[]).length){
      root.innerHTML='<div class="empty"><b>100% VIRTUAL CASH</b><br>'+esc(state.note||'Risk check active.')+'<br><span class="muted">Created '+esc((state.createdAt||'').slice(0,16).replace('T',' '))+'</span></div>';
      return;
    }
    ...
    '<div class="notice" style="margin-top:10px"><strong>VIRTUAL ONLY:</strong> values are updated using verified proxy prices when available. No real orders, broker connection or automatic execution.</div>';
  }
