...
assert.match(mobile,/window\.location\.assign/,'refresh must perform an immediate full navigation');
assert.doesNotMatch(mobile,/await\s+refreshServiceWorker\(\)/,'refresh navigation must not wait for service-worker activation');
...
assert.match(sw,/argus-shell-v14/,'PWA shell must be version-bumped for the iOS refresh fix');
